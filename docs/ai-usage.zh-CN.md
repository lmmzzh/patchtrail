...
```text
本轮改代码前，请先在项目根目录运行 zmg task start --goal "<简短任务目标>"。

改完代码后，请运行 zmg check。

zmg check 之后，请运行 zmg handoff。

zmg handoff 之后，请运行 zmg task close。
...
`zmg check` 检查的是改动风险。`zmg handoff` 会把最近一次检查结果整理成一份给下一个 agent 或未来的自己看的 Markdown 交接文档。`zmg task start` 会把任务目标写进检查报告和交接文档。它们都不替代编译、测试和真机验证。
